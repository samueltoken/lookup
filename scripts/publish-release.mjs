...
const REPO = "lookup";
const TARGET_TAG = "v1.2.5";
const TARGET_NAME = "lookup v1.2.5";

const REPAIR_TAGS = ["v1.1.7", "v1.2.0", "v1.2.1", "v1.2.2", "v1.2.3", "v1.2.4"];
const RELEASE_NOTES_DIR = path.resolve("release-notes");
const RELEASE_DIR = path.resolve("release");
const TARGET_ASSETS = ["lookup-Setup-1.2.5.exe", "latest.yml", "lookup-Setup-1.2.5.exe.blockmap"];
...
function getGitCredential() {
  ...
  if (result.status !== 0 || !result.stdout) {
    throw new Error("Unable to read GitHub credential from git credential store.");
  }
  const map = new Map();
  ...
  if (!username || !password) {
    throw new Error("GitHub credential is missing username/password.");
  }
...
async function ensureRepairedReleases(authHeader) {
  ...
  for (const tag of REPAIR_TAGS) {
    ...
    const remoteBody = String(release.body || "");
    if (!hasBrokenText(remoteBody) && remoteBody.trim() === body.trim()) {
      continue;
    }
...
async function createOrUpdateTargetRelease(authHeader) {
  ...
  if (existing?.id) {
    return await requestJson(authHeader, "PATCH", `/repos/${OWNER}/${REPO}/releases/${existing.id}`, {
      name: TARGET_NAME,
      body,
      draft: false,
      prerelease: false
...
async function uploadAssets(authHeader, release) {
  if (!release?.id || !release?.upload_url) {
    throw new Error("Unable to find release upload URL.");
  }
  ...
  for (const fileName of TARGET_ASSETS) {
    ...
    if (!fs.existsSync(filePath)) {
      throw new Error(`Release asset file missing: ${fileName}`);
    }
